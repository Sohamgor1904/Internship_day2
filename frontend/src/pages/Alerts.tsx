    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex">
      ...
      <main className={`flex-1 transition-all duration-300 p-8 ${sidebarCollapsed ? "ml-16" : "ml-64"}`}>
        ...
        <div className="flex items-center justify-between pb-6 border-b border-slate-200 animate-fade-in-up">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Threat Investigator Panel</h1>
            <p className="text-xs text-slate-500 mt-1">Review, filter, and inspect stateful OCSF telemetry alerts.</p>
          </div>
          <button
            onClick={() => refetch()}
          ...
          >
            <RefreshCw className={`w-4 h-4 ${isFetching ? "animate-spin text-indigo-650" : ""}`} />
          </button>
        </div>

        {/* Filter Toolbar Grid */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-8 p-4 bg-white rounded-xl border border-slate-200 shadow-sm items-center animate-fade-in-up animation-delay-100">
          <div className="md:col-span-2 relative">
            ...
            <Input 
              ...
              className="pl-9 bg-slate-50 border-slate-200 text-slate-800 focus-visible:ring-indigo-600 focus-visible:ring-offset-0 focus-visible:ring-1"
            />
          </div>

          <div>
            <Select value={protocolFilter} onValueChange={setProtocolFilter}>
              <SelectTrigger className="bg-slate-50 border-slate-200 text-slate-700 focus:ring-indigo-600 focus:ring-offset-0 focus:ring-1">
        ...
        {/* Alerts Table */}
        <div className="mt-6 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden animate-fade-in-up animation-delay-200">
          <Table>
            <TableHeader className="bg-slate-50 border-b border-slate-200">
              <TableRow className="hover:bg-transparent border-slate-200">
                <TableHead className="text-slate-500 font-semibold py-4">Timestamp</TableHead>
                <TableHead className="text-slate-500 font-semibold">Source IP</TableHead>
                <TableHead className="text-slate-500 font-semibold">Destination IP</TableHead>
                <TableHead className="text-slate-500 font-semibold">Protocol</TableHead>
                <TableHead className="text-slate-500 font-semibold">Flagged Layer</TableHead>
                <TableHead className="text-slate-500 font-semibold">Confidence</TableHead>
                <TableHead className="text-slate-500 font-semibold">Top SHAP Feature</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredAlerts.length > 0 ? (
                filteredAlerts.map((alert) => (
                  <TableRow 
                    ...
                    className="cursor-pointer border-slate-200 hover:bg-slate-50 transition-colors"
                  >
                    <TableCell className="font-mono text-xs text-slate-600 py-4.5">
